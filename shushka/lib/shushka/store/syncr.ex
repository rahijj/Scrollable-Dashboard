defmodule Shushka.Store.Syncr do
	use GenServer
		...
		case Postgrex.query(
			...
			"SELECT sync_state from sync_store where id=$1", [id]) do
				...
				{:ok, resp} ->
					...
					case Postgrex.query(Shushka.DB, "SELECT path, value, time, type, client_id FROM writes WHERE id=$1 ORDER BY time desc limit $2", [id, 50]) do
						{:ok, writes_resp} ->
