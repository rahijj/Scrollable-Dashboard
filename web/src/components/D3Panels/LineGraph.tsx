	...
	function Line(index: number) {
			...
			if (iter === 0) {
				linearScale = d3
					...
					.domain([
						Math.min(
							-45,
							1.3 * Number(min(Object.values(data_obj)))
						),
						Math.max(
							15,
							1.3 * Number(max(Object.values(data_obj)))
						),
					])
					.range([innerHeight, 0])
				bandScale = d3
		...
		if (filter_List.type !== "None") {
			g.selectAll(".legend").attr(
				"transform",
				`translate(${
					bandScale(new Date(2020, 11, 1)) - 130
				},${linearScale(linearScale.domain()[0] + 14)})`
			)
		} else {
			g.selectAll(".legend").attr(
				"transform",
		...
		combinedList = helper.flattenNodes({
			targetList: combinedList,
			origList: passedList,
		})
