...
const Dashboard: React.FunctionComponent<P> = (props) => {
	...
	useEffect(() => {
		if (window.matchMedia('(max-width: 767px)').matches) {
			setHeadHeight(document.getElementsByClassName('logo-header-logo')[0].clientHeight);
		...
		window.onresize = () => {
			if (window.matchMedia('(max-width: 767px)').matches) {
				...
				if ((window.innerHeight - (height + headHeight)) / (height + headHeight) > 0.05) {
					setHeight(window.innerHeight - headHeight);
				} else if ((height + headHeight - window.innerHeight) / (height + headHeight) > 0.3) {
					setHeight(window.innerHeight - headHeight);
				}
			} else {
				set_openNav('94px');
